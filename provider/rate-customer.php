...
if (!$offerId) {
    header("Location: won-jobs.php");
    exit;
}

// İş Detaylarını ve Geçerliliğini Kontrol Et
$stmt = $pdo->prepare("
    SELECT 
        o.id as offer_id, o.price,
        d.id as demand_id, d.title as demand_title, d.created_at as demand_date,
        u.id as customer_id, u.first_name, u.last_name, u.avatar_url,
    ...
    WHERE o.id = :offer_id AND o.user_id = :reviewer_id_check AND o.status = 'accepted'
");
$stmt->execute(['offer_id' => $offerId, 'reviewer_id' => $userId, 'reviewer_id_check' => $userId]);
$job = $stmt->fetch();

if (!$job) {
    die("İş bulunamadı veya bu işlemi yapmaya yetkiniz yok.");
}

if ($job['existing_review_id']) {
    header("Location: won-jobs.php?msg=already_rated");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $ratingPrice = $_POST['rating_price'] ?? 0; // Müşteri için "Ödeme Kolaylığı" gibi düşünülebilir
    ...
    if ($rating > 0) {
        try {
            ...
            $stmt->execute([$offerId, $userId, $job['customer_id'], $rating, $criteria, $comment]);

            header("Location: won-jobs.php?msg=rated_success");
            exit;
        } catch (Exception $e) {
...
$pageTitle = "Müşteriyi Değerlendir";
$pathPrefix = '../';
    ...
    <div class="layout-content-container flex flex-col max-w-[800px] flex-1">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-slate-500 dark:text-slate-400 text-sm font-medium flex items-center gap-1">
                        ...
                        <?= htmlspecialchars($job['first_name'] . ' ' . $job['last_name']) ?> (Müşteri)
                    </p>
                </div>
            </div>
            <div class="text-left sm:text-right w-full sm:w-auto border-t sm:border-t-0 border-slate-100 pt-4 sm:pt-0">
                <p class="text-xs text-slate-500 dark:text-slate-500 uppercase tracking-wider font-bold">İş Tarihi</p>
        ...
        <form method="POST" class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
            <div class="px-8 py-6 border-b border-slate-100 dark:border-slate-800">
                ...
                <p class="text-slate-500 text-sm">Müşteri ile olan çalışma deneyiminizi puanlayın.</p>
            </div>
            ...
            <div class="p-8 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 pt-8 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <!-- Nezaket / Süreç -->
                    <div class="flex flex-col items-center gap-2">
                        <span class="text-slate-900 dark:text-slate-400 text-sm font-bold">Nezaket</span>
                    ...
                    <!-- Ödeme / Güven -->
                    <div class="flex flex-col items-center gap-2">
                        <span class="text-slate-900 dark:text-slate-400 text-sm font-bold">Ödeme/Güven</span>
                ...
                <div class="flex flex-col gap-3 pt-6">
                    ...
                    <textarea name="comment" class="w-full rounded-xl border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-white p-4 focus:ring-primary focus:border-primary resize-none placeholder:text-slate-400" id="comment" placeholder="Müşteri ile çalışma deneyiminizi paylaşın..." rows="4"></textarea>
                </div>
            </div>
